  ...
  return (
    <div className={`${bgColor} ${textColor} w-full p-16`}>
      ...
      <p className="text-[20px]">
        ...
        needs, then click the "See Pricing" button. We'll show you both low and
        high-ended estimates for each service category, so you'll have an
        overall range of how much it might cost to build such a website.
      </p>
      <div className="py-14 space-y-10 mx-auto">
        {slidersData.map((sliderItem, index) => {
          return (
            <FlexLayout justify="justify-between" key={index}>
              <p className="font-sans text-[24px] font-semibold text-white">
                {sliderItem.title}
              </p>
              <Slider
        ...
        <FlexLayout justify="justify-end">
          <div className="md:text-[20px] w-[50%] ">
            <Button
              className="w-full py-3.5 px-5 font-semibold text-sm"
              bgColor="bg-black"
