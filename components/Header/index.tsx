...
const Header: FC = () => {
  return (
    <div
      className="flex relative  bg-white shadow-sm  justify-between 
    ...
    >
      ...
      <div className="flex gap-x-[12px]">
        <Muscled />
        <p
          className="text-black-80 text-[10px] 
        leading-[10px] font-[500] w-[108px] xl_2:block hidden"
        >
      ...
      <div className="flex items-center">
        <div
          className="h-[100px]  text-sm xl:text-md justify-center 
      hidden lg:flex gap-x-[15px]"
          ...
          <NavItem navTitle="Services">
            <div className="flex justify-between w-full gap-x-[16px]">
              {SeoLeadDataSubCatData.map((subCategory, index) => {
                return (
                  <NavSubItem
                    key={`${subCategory.subCategoryTitle}-1-${index}`}
                    {...subCategory}
                  />
                );
              })}
          ...
          <NavItem navTitle="Learn">
            <div className="flex justify-between w-full gap-x-[16px]">
              {SeoLeadDataSubCatData.map((subCategory, index) => {
                return (
                  <NavSubItem
                    key={`${subCategory.subCategoryTitle}-2-${index}`}
                    {...subCategory}
                  />
                );
              })}
          ...
          <NavItem navTitle="Who We Are">
            <div className="flex justify-between w-full gap-x-[16px]">
              {SeoLeadDataSubCatData.map((subCategory, index) => {
                return (
                  <NavSubItem
                    key={`${subCategory.subCategoryTitle}-3-${index}`}
                    {...subCategory}
                  />
                );
              })}
        ...
        <Link href="/proposal-form">
          <a className="no-underline">
            <button
              className="bg-black rounded-[35px]  justify-center items-center
