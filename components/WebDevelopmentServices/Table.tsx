...
const Table:FC<tableContentProps> = ({
  ...
  return (
    <div className={`flex flex-col rounded-lg ${tableWidth || "w-[400px]"} border-[#BDBDBD] border-[1px]
     border-t-0`}>
...
<div className="flex rounded-t-lg rounded-r-lg space-x-4 bg-[#BDBDBD] pt-[16px] pb-[16px] pl-[28px] ">
    ...
    <div className={`cursor-pointer transition-transform ${
            isOpen ? "rotate-0" : "-rotate-180"} `} onClick={()=>{
        setOpen(state=>!state);
    }}>
...
rounded-b-lg ${subSection?"block":"hidden"}`}>
    ...
    <a className='text-xl text-[#13C47F] font-[500] underline'>{subSection?.title}</a>
    </Link>
</div>

<div className={`${isOpen?"h-fit opacity-1":"h-0 opacity-0"} transition-all duration-200 ease-linear`}>
    {List}
</div>
