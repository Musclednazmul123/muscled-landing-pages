...
const FormField:FC<FormDataProps> = ({name,label,validation,placeholder}) => {
    const { register, watch, formState: { errors } } = useFormContext();
  return (
    <div className='flex flex-col gap-y-[15px] w-[240px]'>
          <div className='flex gap-x-[10px] items-center'>
            <label className='text-[16px] font-semibold text-black-70'>{label}</label>
          ...
          <input {...register(name, validation || {} )}
           ...
           ${errors[name]?"border-b-[#C40000] "
           :"border-b-black-50 focus:border-b-black-50"} bg-inherit pb-[20px]`}
